...
def training(
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        image, viewspace_point_tensor, visibility_filter, radii, objects = (
            render_pkg["render"],
            render_pkg["viewspace_points"],
            render_pkg["visibility_filter"],
            render_pkg["radii"],
            render_pkg["render_object"],
        )
        ...
        if getattr(opt, "use_uncertainty_2d", False):
            ...
            loss_obj = weighted_2d_ce_loss(
                ...
                weight_map,
                num_classes,
                min_weight=getattr(opt, "entropy_min_weight", 0.5),
            )
        else:
            loss_obj = (
        ...
        if iteration % opt.reg3d_interval == 0:
            ...
            if getattr(opt, "use_aniso", False):
                common_kwargs = dict(
                    ...
                    neg_weight=getattr(opt, "reg3d_neg_weight", 0.0),
                    neg_k=getattr(opt, "reg3d_neg_k", 2),
                    neg_margin=getattr(opt, "reg3d_neg_margin", 0.2),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_iterations", nargs="+", type=int, default=[1_000, 7_000, 30_000, 30_000]
    )
    parser.add_argument(
        "--save_iterations", nargs="+", type=int, default=[1_000, 7_000, 30_000, 60_000]
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use wandb to record loss value",
    )
